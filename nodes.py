    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "tau": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 4.0, "step": 0.05,
                                  "tooltip": "稀疏路由阈值 β：越大越稀疏越快。1.0 ≈ 26% 密度（V100 fp16 实测画质肉眼无损），1.5 ≈ 更低密度"}),
                ...
                "end_percent": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01,
                                          "tooltip": "采样进度高于此比例走 dense（1.0=不启用尾部 dense，与 43s/步 实测一致；可调 0.9 保尾部质量）"}),
                "min_tokens": ("INT", {"default": 1024, "min": 64, "max": 65536, "step": 64,
                                       "tooltip": "序列短于此 token 数不走稀疏（直接 SDPA）"}),
                "dense_blocks": ("STRING", {"default": "0-1",
